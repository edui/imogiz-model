...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='U-Net PyTorch Model for Height Prediction in  Dataset')
    ...
    parser.add_argument('-ls', '--loss', type=str,
                        required=True, help='Height loss type')
    parser.add_argument('-d', '--dataset', type=str,
                        required=True, help='Dataset directory')

    parser.add_argument('-w1', '--w1_loss', type=float,
                        default=1, help='Loss weight for Height Estimation')
    parser.add_argument('-w2', '--w2_loss', type=float,
                        default=1, help='Loss weight for Cross Entropy')
    parser.add_argument('-w3', '--w3_loss', type=float,
                        default=1, help='Loss weight for Dice Loss')

    parser.add_argument('-m', '--min_neuron', type=int, default=128,
                        help='Minimum neuron number for the first layer')
    parser.add_argument('-bs', '--batch_size', type=int,
                        default=16, help='Batch size')
    parser.add_argument('-r', '--resume', type=int,
                        required=True, help='Continue training')
    parser.add_argument('-pr', '--pretrained', type=str,
    ...
    train = DataLoader(Images(args.dataset, 'TRAINING.csv', False),
                       batch_size=args.batch_size, num_workers=8, shuffle=True)

    valid = DataLoader(Images(args.dataset, 'VALIDATION.csv', False),
    ...
    print('Trainable params:', sum(p.numel()
                                   for p in net.parameters() if p.requires_grad))
    print('Non-trainable params:', n_params-sum(p.numel()
                                                for p in net.parameters() if p.requires_grad))
    ...
    for ep in range(start_epoch, start_epoch+n_epochs):
        ...
        with tqdm(total=len(train), dynamic_ncols=True) as progress:
            ...
            for idx, batch_data in enumerate(train):
                #X, y_mask, y_joint, y_height = batch_data['img'].cuda(), batch_data['mask'].cuda(
                #), batch_data['joint'].cuda(), batch_data['height'].cuda()
                X, y_mask,  y_height = batch_data['img'], batch_data['mask'], batch_data['height']

                optimizer.zero_grad()

                #mask_o, joint_o, height_o = net(X)
                mask_o, height_o = net(X)

                loss_m = w1_loss * \
                    (dice_loss(mask_o, y_mask, 0) + dice_loss(mask_o, y_mask, 1))/2
                ...
                #                     joint=tj_/(idx+1), height=th_/(idx+1), acc=acc_/(idx+1))
                progress.set_postfix(loss=loss_/(idx+1), mask=tm_/(idx+1), height=th_/(idx+1), acc=acc_/(idx+1))
        ...
        if vl_ < best_val:
            ...
            if os.path.exists('models/' + SAVE_DIR):
                os.remove('models/' + SAVE_DIR +
                          'model_ep_{}.pth.tar'.format(best_ep))
            else:
                os.makedirs('models/' + SAVE_DIR)

            torch.save(state, 'models/' + SAVE_DIR +
                       'model_ep_{}.pth.tar'.format(ep+1))
            best_ep = ep+1

        progress.write('T Loss: {:.3f} - T Mask: {:.3f} - T Joint: {:.3f} - T Height: {:.3f}\nV Loss: {:.3f} - V Mask: {:.3f} - V Joint: {:.3f} - V Height: {:.3f}'.format(
            loss_, tm_, tj_, th_, vl_, vm_, vj_, vh_))
    ...
    torch.save(state, 'models/' + SAVE_DIR +
               'last_model_ep_{}.pth.tar'.format(start_epoch+n_epochs))
